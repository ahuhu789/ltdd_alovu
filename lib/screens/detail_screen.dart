import 'dart:async';
import 'dart:convert'; // BẮT BUỘC: Thêm thư viện này để giải mã chuỗi Base64
...
class _DetailScreenState extends State<DetailScreen> {
    ...
    FirebaseFirestore.instance
        ...
        .listen((doc) {
          if (doc.exists && mounted) {
            setState(() {
              _currentRating = (doc.data()?['rating'] ?? widget.field.rating)
                  .toDouble();
            });
          }
        });
  ...
  /// BỘ LỌC ĐỘNG TOÀN DIỆN: Phân loại thông minh Base64, Asset và Network URL (Chống lỗi cache / nhận diện sai)
  ImageProvider? _buildAvatarImage(String avatarUrl) {
    if (avatarUrl.isEmpty) return null;

    // 1. Phải ưu tiên kiểm tra Base64 TRƯỚC (Dành cho ảnh bạn vừa đổi thành công trên máy ảo)
    ...
    // 2. Kiểm tra link mạng URL thông thường
    if (avatarUrl.startsWith('http') || avatarUrl.startsWith('https')) {
      return NetworkImage(avatarUrl);
    }

    // 3. Kiểm tra ảnh hệ thống / Asset cục bộ mặc định
    if (avatarUrl.startsWith('assets/') ||
        avatarUrl == "2" ||
        avatarUrl == "avatar_pixel") {
      if (avatarUrl == "2" || avatarUrl == "avatar_pixel") {
        return const AssetImage('assets/images/avatar_pixel.png');
      }
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // 🛠️ SỬA CHÍNH TẠI ĐÂY: Dùng FutureBuilder truy vấn trực tiếp vào bảng 'users' realtime theo ID người viết bài
            FutureBuilder<DocumentSnapshot>(
              ...
              builder: (context, userSnapshot) {
                String dynamicAvatarUrl = review.avatarUrl;
                String dynamicUserName = review.userName;

                if (userSnapshot.hasData && userSnapshot.data!.exists) {
                  final uData =
                      userSnapshot.data!.data() as Map<String, dynamic>?;
                  // Luôn ưu tiên lấy avatar và tên mới nhất từ trang cá nhân thật
                  dynamicAvatarUrl = uData?['avatar'] ?? review.avatarUrl;
